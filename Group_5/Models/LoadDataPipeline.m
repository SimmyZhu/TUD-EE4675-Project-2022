...
%caseNum = nan(length(theFiles),1);
caseNum = strings(length(theFiles),1);
...
%     drawnow
    ...
    %Determine variance over columns of spectrogram
    variances = var(Data_spec_MTI2);
    %Smooth met moving avarage
    smoothvar = movmean(variances,50);  %maybe change to 100? or higher?
    %Get part likely to be most interesting
    [~, interest] = max(smoothvar);
    
    %Cut out intersting part
    if size(Data_spec_MTI2,2) - interest < 481 
        Data_spec_MTI2 = Data_spec_MTI2(:,end-480:end);
    elseif interest < 481
        Data_spec_MTI2 = Data_spec_MTI2(:,1:481);
    else
        Data_spec_MTI2 = Data_spec_MTI2(:,interest - 240: interest + 240);
    end

    
    CompleteMat(k,:,:) = Data_spec_MTI2;
    caseNum(k) = baseFileName(5:6); %str2double(baseFileName(end-1:end));
