...
pred_logits_val=model.predict(test_points)
pred_labels_val = np.argmax(pred_logits_val, axis = 1).reshape(-1,1)

pred_logits_train=model.predict(train_points)
pred_labels_train = np.argmax(pred_logits_train, axis = 1).reshape(-1,1)

confusion_mat_train = confusion_matrix(train_labels, pred_labels_train)
f1_score_all_train = f1_score(train_labels, pred_labels_train, average='macro')
f1_score_individual_train = f1_score(train_labels, pred_labels_train, average=None)

print(confusion_mat_train)
print(f1_score_all_train)
print(f1_score_individual_train)

score, acc = model.evaluate(train_points, train_labels,
                            batch_size=32)
print('Train score:', score)
print('Train accuracy:', acc)

confusion_mat_val = confusion_matrix(test_labels, pred_labels_val)
f1_score_all_val = f1_score(test_labels, pred_labels_val, average='macro')
f1_score_individual_val = f1_score(test_labels, pred_labels_val, average=None)

print(confusion_mat_val)
print(f1_score_all_val)
print(f1_score_individual_val)

fig, ax = plt.subplots()
ax = sns.heatmap(confusion_mat_val, annot=True, cmap='Blues')
